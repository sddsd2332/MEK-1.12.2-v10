...
public abstract class TileEntityCultivateElectricMachine<RECIPE extends CultivateMachineRecipe<RECIPE>>
    ...
    public TileEntityCultivateElectricMachine(String soundPath, BlockStateMachine.MachineType type, int ticksRequired,
        ...
        configComponent.addOutput(TransmissionType.ITEM, new SideData("Extra2", EnumColor.ORANGE, new int[] { 2 }));
    ...
    public RECIPE getRecipe() {
        CultivateMachineInput input = getInput();
        if (cachedRecipe == null || !input.testEquality(cachedRecipe.getInput())) {
            cachedRecipe = RecipeHandler.getCellCultivateRecipe(input, getRecipes());
        }
        return cachedRecipe;
    }
