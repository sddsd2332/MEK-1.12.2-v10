...
public class TileEntityVoid extends TileEntityBasicMachine<CompositeProbabilityInput, CompositeProbabilityOutput, VoidRecipe> implements IFluidHandlerWrapper, IGasHandler,
        ISustainedData, ITankManager {

    private static final String[] methods = new String[]{"getEnergy", "getProgress", "isActive", "facing", "canOperate", "getMaxEnergy", "getEnergyNeeded",
            "getFluidStored", "getGasStored"};

    public double itemchane;

    public double inputFluidchane;

    public double inputGaschane;
    ...
    public FluidTank outputFluidTank = new FluidTank(10000);
    public GasTank outputGasTank = new GasTank(10000);

    public TileEntityVoid() {
        super("prc", MachineType.VOID, 3, 100, new ResourceLocation(Mekanism.MODID, "gui/GuiVoidExcavator.png"));
        ...
        configComponent.addOutput(TransmissionType.FLUID, new SideData("Output", EnumColor.INDIGO, new int[]{1}));
        ...
        configComponent.addOutput(TransmissionType.GAS, new SideData("Output", EnumColor.INDIGO, new int[]{3}));
    ...
    public boolean isItemValidForSlot(int slotID, @Nonnull ItemStack itemstack) {
        if (slotID == 0) {
            return RecipeHandler.isInVoidRecipe(itemstack);
        } else if (slotID == 1) {
    ...
    @Override
    public VoidRecipe getRecipe() {
        CompositeProbabilityInput input = getInput();
        if (cachedRecipe == null || !input.testEquality(cachedRecipe.getInput())) {
            cachedRecipe = RecipeHandler.getVoidRecipe(input);
        }
        return cachedRecipe;
    }

    @Override
    public CompositeProbabilityInput getInput() {
        if (cachedRecipe == null || world.provider.getDimension() != cachedDimensionId) {
            cachedDimensionId = world.provider.getDimension();
            cachedRecipe = RecipeHandler.getVoidRecipe(new CompositeProbabilityInput(cachedDimensionId, inventory.get(0), itemchane, inputFluidTank.getFluid(), inputFluidchane, inputGasTank.getGas(), inputGaschane
            ));
        }
        return new CompositeProbabilityInput(cachedDimensionId, inventory.get(0), itemchane, inputFluidTank.getFluid(), inputFluidchane, inputGasTank.getGas(), inputGaschane);
    }

    @Override
    public void operate(VoidRecipe recipe) {
        recipe.operate(cachedDimensionId, inventory, inputFluidTank, inputGasTank, outputFluidTank, outputGasTank);
        markDirty();
    }

    @Override
    public boolean canOperate(VoidRecipe recipe) {
        return recipe != null && recipe.canOperate(cachedDimensionId, inventory, inputFluidTank, inputGasTank, outputFluidTank, outputGasTank);
    }
    ...
    @Override
    public Map<CompositeProbabilityInput, VoidRecipe> getRecipes() {
        return null;
    }
    ...
    public boolean canDrain(EnumFacing from, @Nullable FluidStack fluid) {
        return configComponent.getOutput(TransmissionType.FLUID, from, facing).hasSlot(1) && FluidContainerUtils.canDrain(outputFluidTank.getFluid(), fluid);
    }
    ...
    public boolean canReceiveGas(EnumFacing side, Gas type) {
        return configComponent.getOutput(TransmissionType.GAS, side, facing).hasSlot(2) && inputGasTank.canReceive(type);
    }

    @Override
    public boolean canDrawGas(EnumFacing side, Gas type) {
        return configComponent.getOutput(TransmissionType.GAS, side, facing).hasSlot(3) && outputGasTank.canDraw(type);
    }
