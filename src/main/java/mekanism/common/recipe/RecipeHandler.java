...
public final class RecipeHandler {
    ...
    public static void addVoidRecipe(int dimensionID, ItemStack inputSolid, double itemChane,
                                     FluidStack inputFluid, double inputFluidChane,
                                     GasStack inputGas, double inputGasChane,
                                     ItemStack outputSolid, double outputitemChane,
                                     FluidStack outputFluid, double outputFluidChane,
                                     GasStack outputGas, double outputGasChane,
                                     double extraEnergy, int ticks) {
        addRecipe(Recipe.VOID, new VoidRecipe(dimensionID, inputSolid, itemChane, inputFluid, inputFluidChane, inputGas, inputGasChane,
                outputSolid, outputitemChane,
                outputFluid, outputFluidChane,
                outputGas, outputGasChane, extraEnergy, ticks));
    }
    ...
    @Nullable
    public static VoidRecipe getVoidRecipe(@Nonnull CompositeProbabilityInput input) {
        return getRecipe(input, Recipe.VOID);
    }
    ...
    public static boolean isInVoidRecipe(@Nonnull ItemStack stack) {
        if (!stack.isEmpty()) {
            for (CompositeProbabilityInput key : Recipe.VOID.get().keySet()) {
                if (key.containsType(stack)) {
                    return true;
                }
            }
    ...
    public static class Recipe<INPUT extends MachineInput<INPUT>, OUTPUT extends MachineOutput<OUTPUT>, RECIPE extends MachineRecipe<INPUT, OUTPUT, RECIPE>> {
        // Don't let IDEA format this, otherwise the game won't run
        private static List<Recipe> values = new ArrayList<>();
        // Information on the recipe needs to be obtained here, don't let IDEA move this
        ...
        public static final Recipe<CompositeProbabilityInput, CompositeProbabilityOutput, VoidRecipe> VOID = new Recipe<>(
                MachineType.VOID, CompositeProbabilityInput.class, CompositeProbabilityOutput.class, VoidRecipe.class);
        ...
        public boolean containsRecipe(Fluid input) {
            ...
            for (Entry<INPUT, RECIPE> entry : recipes.entrySet()) {
                ...
                if (entry.getKey() instanceof CompositeProbabilityInput) {
                    if (((CompositeProbabilityInput) entry.getKey()).theFluid.getFluid() == input) {
                        return true;
                    }
                }
            }
