...
public class GuiFactory extends GuiMekanismTile<TileEntityFactory> {
    ...
    protected void drawGuiContainerBackgroundLayer(int xAxis, int yAxis) {
        ...
        int xOffset = tileEntity.tier == FactoryTier.BASIC ? 59 : tileEntity.tier == FactoryTier.ADVANCED ? 39 : tileEntity.tier == FactoryTier.ULTIMATE ? 31 : 31;
        ...
        if (tileEntity.getRecipeType().getFuelType() == MachineFuelType.ADVANCED) {
            if (tileEntity.getScaledGasLevel(160) > 0) {
                ...
                if (gas != null) {
                    ...
                    displayGauge(26, 77, tileEntity.getScaledGasLevel(160), 5, gas.getGas().getSprite());
                    MekanismRenderer.resetColor();
                }
            }
        } else if (tileEntity.getRecipeType() == RecipeType.INFUSING) {
            if (tileEntity.getScaledInfuseLevel(160) > 0) {
                displayGauge(26, 77, tileEntity.getScaledInfuseLevel(160), 5, tileEntity.infuseStored.getType().sprite);
            }
        }
    }
    ...
    protected void mouseClicked(int x, int y, int button) throws IOException {
        ...
        if (button == 0 || Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
            int xAxis = x - guiLeft;
            int yAxis = y - guiTop;
            if (xAxis > 26 && xAxis < 197 && yAxis > 77 && yAxis < 81) {
