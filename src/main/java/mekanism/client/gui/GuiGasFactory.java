...
public class GuiGasFactory extends GuiMekanismTile<TileEntityFactory> {

    private GuiButton infuserDumpButton = null;

    public GuiGasFactory(InventoryPlayer inventory, TileEntityFactory tile) {
...
//隐藏气体，流体显示开始
        if(tile.tier == FactoryTier.BASIC) {//基础工厂
            if (GuiGasFactory.this.tileEntity.getRecipeType() == RecipeType.FARM) {
                addGuiElement(new GuiStorage(GuiStorage.StorageType.BASIC_FACTORY, this, resource, 7, 77));
            } else if(GuiGasFactory.this.tileEntity.getRecipeType() == RecipeType.SAWING) {
                addGuiElement(new GuiStorage(GuiStorage.StorageType.BASIC_FACTORY, this, resource, 7, 77));
                addGuiElement(new GuiStorage(GuiStorage.StorageType.OVERLAY, this, resource, 7, 95));
            } else if (GuiGasFactory.this.tileEntity.getRecipeType() == RecipeType.COMPRESSING || GuiGasFactory.this.tileEntity.getRecipeType() == RecipeType.PURIFYING || GuiGasFactory.this.tileEntity.getRecipeType() == RecipeType.INJECTING || GuiGasFactory.this.tileEntity.getRecipeType() == RecipeType.INFUSING) {
...
//输入输出插槽开始
        if(tile.tier == FactoryTier.BASIC){//基础
            ...
            if (GuiGasFactory.this.tileEntity.getRecipeType() == RecipeType.SAWING || GuiGasFactory.this.tileEntity.getRecipeType() == RecipeType.FARM) {
    ...
    public void initGui() {
        ...
        this.buttonList.add(this.infuserDumpButton = new GuiButtonImage(1, this.guiLeft+6, this.guiTop+44, 21, 10, 147, 72, 0, MekanismUtils.getResource(ResourceType.GUI, "GuiMetallurgicInfuser.png")){
            ...
            public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
                if (GuiGasFactory.this.tileEntity.getRecipeType() == RecipeType.INFUSING) {
            ...
            public boolean mousePressed(Minecraft mc, int mouseX, int mouseY) {
                return GuiGasFactory.this.tileEntity.getRecipeType() == RecipeType.INFUSING && super.mousePressed(mc, mouseX, mouseY);
            }
        });
    }
