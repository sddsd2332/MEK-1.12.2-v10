...
public class GuiPowerBarLog extends GuiElement {
    ...
    public GuiPowerBarLog(IGuiWrapper gui, IPowerInfoHandler h, ResourceLocation def, int x, int y) {
        super(MekanismUtils.getResource(ResourceType.GUI_ELEMENT, "GuiPowerBarLog.png"), gui, def);
        tileEntity = null;
        handler = h;
    ...
    public void renderBackground(int xAxis, int yAxis, int guiWidth, int guiHeight) {
        ...
        if (handler.getLevel() > 0) {
            int displayInt = (int) (handler.getLevel() * 79) + 2;
